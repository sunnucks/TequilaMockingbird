...
public class MainProgram {
	...
	public static void main(String[] args) {
		...
		Reservable[] reservables = new Reservable[items.length];
		int pos = 0;
		for(Item item : items) {
			if(item instanceof Reservable) {
				reservables[pos++] = (Reservable)item;
			}
		}
		//Test
		boolean test = reservables[0].reserveItemFor(member1);
		System.out.println("reservable book not yet borrowed... "+test);
		
		items[0].borrowItemBy(member1);
		boolean test2 = reservables[1].canReserve(member1);
		System.out.println("reservable book that is borrowed... "+test2);
